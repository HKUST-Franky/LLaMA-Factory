...
# 读取原始 JSON 文件
with open("/root/LLaMA-Factory/wxh_scripts/selected_profiles1.json", "r") as f:
    profile = json.load(f)
...
random_dpo_data = random.sample(dpo_data, 1000)

# 写入 JSON 文件
with open("/root/LLaMA-Factory/data/rolellm_dpo_all.json", "w", encoding="utf-8") as f:
    json.dump(random_dpo_data, f, ensure_ascii=False, indent=2)

print(len(random_dpo_data))